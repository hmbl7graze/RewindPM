...
public class EventPublishingEventStoreDecoratorTests
{
    ...
    public async Task HasEventsAsync_PassesCancellationToken()
    {
        ...
        _mockInnerEventStore.HasEventsAsync(cts.Token)
            .Returns(expectedResult);

        // Act
        var result = await _decorator.HasEventsAsync(cts.Token);

        // Assert
        Assert.Equal(expectedResult, result);
        await _mockInnerEventStore.Received(1).HasEventsAsync(cts.Token);
    }
    ...
    {
        // Arrange
        var expectedResult = new List<(string EventType, string EventData)>
        {
            ("TaskCreated", "{\"AggregateId\":\"...\"}"),
            ("TaskStatusChanged", "{\"AggregateId\":\"...\"}")
        };
    ...
    public async Task GetAllEventsAsync_PassesCancellationToken()
    {
        ...
        _mockInnerEventStore.GetAllEventsAsync(cts.Token)
            .Returns(expectedResult);

        // Act
        var result = await _decorator.GetAllEventsAsync(cts.Token);

        // Assert
        Assert.Equal(expectedResult, result);
        await _mockInnerEventStore.Received(1).GetAllEventsAsync(cts.Token);
    }
