...
└─────────────────────────────────────────┘
              ↓
┌─────────────────────────────────────────┐
│ Application Layer (RewindPM.Application)│
...
**原則:**
- イベントハンドラーでReadModelを更新
- Event Store → Read Model は一方向
...
public class TaskEventHandler
{
    public async Task Handle(TaskCreated @event)
    {
        ...
        await _readModelContext.Tasks.AddAsync(task);
...
Components/
├─ Modal.razor (汎用)
├─ GanttChart.razor (ガントチャート)
├─ TimelineSlider.razor (タイムトラベル)
├─ TaskCard.razor
└─ TaskDetailModal.razor
```

**注意:** KanbanBoard.razorは将来実装
